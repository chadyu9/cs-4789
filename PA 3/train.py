...
def experiment(args):
    ...
    action_dim = (
        env.action_space.n
    )  # here it is discrete, so we have n here as opposed to the dimension of the action
    ...
    for i in tqdm(range(args.num_episodes)):
        ...
        for t in count():
            ...
            if len(buffer) >= args.batch_size:
                if t % args.learning_freq == 0:
                    ...
                    qs = network(states).gather(1, actions.unsqueeze(1)).squeeze()

                    # COMPUTE TARGET Q VALUES FROM BATCH HERE
                    if args.target:
                        target_qs = target_network.get_targets(
                            rewards, next_states, dones
                        )
                    else:
                        target_qs = network.get_targets(rewards, next_states, dones)
        ...
        if args.target and (i + 1) % args.target_update_freq == 0:
...
def get_args():
    ...
    # Network args
    parser.add_argument(
        "--hidden_sizes", nargs="+", type=int, help="hidden sizes of Q network"
    )
    parser.add_argument(
        "--lr",
        type=float,
        default=0.00025,
        help="learning rate for Q function optimizer",
    )
    parser.add_argument(
        "--target", action="store_true", help="if we want to use a target network"
    )
    parser.add_argument(
        "--target_update_freq",
        type=int,
        default=10,
        help="how often we update the target network",
    )
    parser.add_argument(
        "--tau", type=float, default=1.0, help="target update parameter"
    )
    ...
    parser.add_argument(
        "--save_path", default="./trained_agent", help="agent save path"
    )
    parser.add_argument(
        "--learning_freq",
        type=int,
        default=1,
        help="how often to update the network after collecting experience",
    )
