...
def compute_softmax(logits, axis):
    ...
    # TODO
    return np.exp(logits - np.max(logits)) / np.sum(
        np.exp(logits - np.max(logits)), axis=axis
    )
...
def compute_action_distribution(theta, phis):
    ...
    # TODO
    return compute_softmax(np.dot(theta.T, phis), axis=1)
...
def compute_log_softmax_grad(theta, phis, action_idx):
    ...
    # TODO
    return phis[:, [action_idx]] - phis @ compute_action_distribution(theta, phis).T
...
def get_args():
    ...
    parser.add_argument(
        "--expert_save_path", default="./learned_policies/NPG/expert_theta.npy"
    )
    parser.add_argument(
        "--num_rollout_steps",
        type=int,
    ...
    parser.add_argument(
        "--dagger_supervision_steps",
        type=int,
        help="number of epochs for supervised learning step within dagger",
    )
    ...
    parser.add_argument(
        ...
        help="policy saving directory",
    )
    parser.add_argument(
        "--state_to_remove", default=None, type=int, help="index of the state to remove"
    )
