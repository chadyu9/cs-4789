...
with open(os.path.join(os.path.dirname(__file__), "test_info.pkl"), "rb") as f:
...
for i in test_cases:
    ...
    assert (
        soln_action_dist.shape == action_dist.shape
    ), "compute_action_dist output shape incorrect"
...
for i in test_cases:
    ...
    assert (
        soln_grad.shape == grad.shape
    ), "compute_log_softmax_grad output shape incorrect"
...
for i in test_cases:
    ...
    assert (
        soln_fisher.shape == fisher.shape
    ), "compute_fisher_matrix output shape incorrect"
...
for i in test_cases:
    ...
    assert (
        soln_v_grad.shape == v_grad.shape
    ), "compute_value_gradient output shape incorrect"
