...
class DDPController:
    def __init__(self, env):
        ...
        self.lf = lambda x: 0.5 * (
            np.square(1.0 - np.cos(x[2])) + np.square(x[1]) + np.square(x[3])
        )  # final cost
        ...
        self.l_x = lambda s, a: self.local_controller.compute_local_expansions(
            s, a, self.f, self.l
        )[5]
        self.l_u = lambda s, a: self.local_controller.compute_local_expansions(
            s, a, self.f, self.l
        )[6]
        self.l_xx = lambda s, a: self.local_controller.compute_local_expansions(
            s, a, self.f, self.l
        )[2]
        self.l_uu = lambda s, a: self.local_controller.compute_local_expansions(
            s, a, self.f, self.l
        )[3]
        self.l_ux = lambda s, a: self.local_controller.compute_local_expansions(
            s, a, self.f, self.l
        )[4]

        # Dynamic
        # from your implementation
        self.f_x = lambda s, a: self.local_controller.compute_local_expansions(
            s, a, self.f, self.l
        )[0]
        self.f_u = lambda s, a: self.local_controller.compute_local_expansions(
            s, a, self.f, self.l
        )[1]
    ...
    def backward(self, x_seq, u_seq):
        ...
        for t in range(self.pred_time - 1, -1, -1):
            ...
            q_x = self.l_x(x_seq[t], u_seq[t]) + np.matmul(
                f_x_t.T, self.v_x[t + 1]
            )  # q_t + A_t^T @ q'_{t+1}
            q_u = self.l_u(x_seq[t], u_seq[t]) + np.matmul(
                f_u_t.T, self.v_x[t + 1]
            )  # r_t + B_t^T @ q'_{t+1}
            q_xx = (
                self.l_xx(x_seq[t], u_seq[t])
                + np.matmul(np.matmul(f_x_t.T, self.v_xx[t + 1]), f_x_t)
                + np.dot(self.v_x[t + 1].T, np.squeeze(self.f_xx(x_seq[t], u_seq[t])))
            )  # Q_t + A_t^T @ Q_{t+1} @ A_t + q_{t+1}^T @ X_t
            tmp = np.matmul(f_u_t.T, self.v_xx[t + 1])  # B_t^T @ Q_{t+1} |A|x|S|
            q_uu = (
                self.l_uu(x_seq[t], u_seq[t])
                + np.matmul(tmp, f_u_t)
                + np.dot(self.v_x[t + 1].T, np.squeeze(self.f_uu(x_seq[t], u_seq[t])))
            )  # R_t + B_t^T @ Q_{t+1} @ B_t + q_{t+1}^T @ Y_t
            q_ux = (
                self.l_ux(x_seq[t], u_seq[t]).T
                + np.matmul(tmp, f_x_t)
                + np.dot(self.v_x[t + 1].T, np.squeeze(self.f_ux(x_seq[t], u_seq[t])))
            )  # M_t + B_t^T @ Q_{t+1} @ A_t + q_{t+1}^T @ Z_t
    ...
    def forward(self, x_seq, u_seq, k_seq, kk_seq):
        ...
        for t in range(T):
            u_seq_hat[t] = u_seq[t] + k_seq[t] + kk_seq[t] @ (x_seq_hat[t] - x_seq[t])
            x_seq_hat[t + 1] = self.f(x_seq_hat[t], u_seq_hat[t])

        # return x_seq_hat and u_seq_hat
        return x_seq_hat, np.clip(u_seq_hat, -self.umax, self.umax)
    ...
    def train(self, init_obs):
        ...
        Parameters:
        init_obs (numpy array) with shape (T,n_s)

        Returns:
            A list, actions that represent the optimal action at each
            timestep. The shape of actions is (T,n_a) (now we set T=400)
        """
        ...
        for i in tqdm(range(400)):
            ...
            x_seq, u_seq = self.forward(np.array(x_seq), np.array(u_seq), k_seq, kk_seq)

            # step the environment with the policy for s_0
            # redefine the s_0 to be the next observation
            x_seq[0] = self.env._step(u_seq[0])[0]
